...
#define SIZEDATA_INDATABLOCK (1024-(4*4))
...
void CZipFile::Open(LPCTSTR pathname, bool append)
{
	...
	{
		if (!zi.filezip.Open(pathname, CFile::modeReadWrite | CFile::shareDenyWrite, e))
			throw e;
		e->Delete();
	...
	zi.filezip.Seek(central_pos, CFile::begin);
	...
	unzlocal_getShort(zi.number_entry);

	unzlocal_getShort(pos_tmp);

	unzlocal_getLong(size_inf);

	unzlocal_getLong(inf_pos);


	zi.filezip.Seek(inf_pos, CFile::begin);
    

	char buf[READBUF];
	if (size_inf>=READBUF)
	{
	for (uInt i=0;(i<(size_inf/READBUF));i++)
	{
		zi.filezip.Read(buf,sizeof(buf));
		zi.central_dir.add_data_in_datablock(buf,sizeof(buf));
	}
	}
    zi.filezip.Read(buf,size_inf%READBUF);
	zi.central_dir.add_data_in_datablock(buf,size_inf%READBUF);
	...
	{
		if (!zi.filezip.Open(pathname, CFile::modeCreate|CFile::modeWrite | CFile::shareDenyWrite, e))
			throw e;
		e->Delete();	
...
uLong CZipFile::unzlocal_SearchCentralDir()
{
	
	uLong uMaxBack=0xffff; /* maximum size of global comment */

	uLong uSizeFile = zi.filezip.GetLength();
	...
	try
	{
		while ((uBackRead < uMaxBack) && !uPosFound)
		{
			...
			zi.filezip.Seek(uReadPos, CFile::begin);
			zi.filezip.Read(buf, uReadSize);
...
void CZipFile::unzlocal_getByte(int & pi)
{
...
void CZipFile::unzlocal_getShort (uLong & pX)
{
    int i;
...
void CZipFile::unzlocal_getLong (uLong & pX)
{

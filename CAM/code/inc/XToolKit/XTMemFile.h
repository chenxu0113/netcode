...
public:
	...
	//
	CXTMemFile(
		// Pointer to the buffer to be attached to CXTMemFile.
		BYTE* lpBuffer,
	...
	CXTMemFile(
		...
		LPCTSTR lpstFilename,
		// A UINT that defines the file’s sharing and access mode. It specifies
	...
	// Destroys a CXTMemFile object, handles cleanup and de-allocation.
	//
    virtual ~CXTMemFile();
...
//:End Ignore
	...
	//
    virtual bool Open(
		// Specifies a NULL terminated string that is the path to the desired file.
		CString strFilename, 
		// Specifies a UINT that defines the sharing and access mode in the file.
		...
		UINT uiOpenFlags, 
		// Specifies a pointer to an existing file-exception object that receives
    ...
    // This member function reads a string.  Returns TRUE if successful,
	// or FALSE if there is an error.
	//
    virtual BOOL ReadString(
		// A CString reference to an object to receive the string that is read.
		CString& rString); 
    ...
    virtual void WriteString( 
		// Specifies a pointer to a buffer containing a null-terminated text
		// string.
		LPCTSTR lpsz );
...
//:End Ignore
	...
	// the information specified in the 'fDuplicate' object. Returns 'true'
	// if successful, otherwise returns 'false'.
	// 
    virtual bool Duplicate(
		// A pointer to a valid CFile object.
		CFile *fDuplicate);

    // This member function will initialize the CXTMemFile object with
	// the information specified in the 'strDup' string object. Returns 'true'
	// if successful, otherwise returns 'false'.
	//
    virtual bool Duplicate(
	...
	// last Flush(). Returns 'true' if successful, otherwise returns 'false'.
	//
    virtual bool Discard();

    // This member function inserts any File and returns the length of
	// the actual copied bytes. Returns a DWORD value that represents the
	// length of the copied bytes.
	//
    virtual DWORD Insert(
		// A pointer to a valid CFile object.
		CFile* fSrc, 
		// Represents the source file position.
		DWORD dwSourcePos, 
		// Represents the destination file position.
		DWORD dwDestPos, 
	...
	//
    virtual DWORD Insert(
		// Specifies a NULL terminated string that is the path to the desired
		// file.
		CString strSrc, 
		// Represents the source file position.
		DWORD dwSourcePos, 
	...
	// of the actual copied bytes. Returns a DWORD value that represents the
	// length of the copied bytes.
	//
    virtual DWORD Extract(
		// A pointer to a valid CFile object.
		CFile *fDest,
    ...
    // This member function finds data in the file. Returns a LONG data
	// type.
	// 
    LONG FindData(
		// Pointer to the buffer to receive the data found.
		void* pData, 
		// Size of the data to find.
		DWORD dwDataLen, 
		// Starting position.
		LONG lStartPos);
	...
	//
    void operator =(
		// DWORD value that specifies file position.
		DWORD dwFilePos);
...
protected:
    ...
    // This member function loads the file into memory. Returns 'true'
	// if successful, otherwise returns 'false'.
	//
    virtual bool Load();  

    // This member function saves the file to disk. Returns 'true' if successful,
	// otherwise returns 'false'.
	//
    virtual bool Save();

    // This member function imports the data of a CFile derived object
	// (operator = ). Returns 'true' if successful, otherwise returns 'false'.
	//
    virtual bool Import(
		// A pointer to a valid CFile object.
		CFile *fImp);

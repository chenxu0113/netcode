...
public:
    ...
    // Destroys a CXTTreeCtrl object, handles cleanup and de-allocation.
    //
    virtual ~CXTTreeCtrl();
...
public:
    ...
    virtual HTREEITEM GetNextItem(
        ...
        HTREEITEM hItem,
        // A flag indicating the type of relation to 'hItem'.  This flag
	...
	// expanded. Returns the item immediately below the reference item.
	//
    virtual HTREEITEM GetNextItem(
	...
	// completely expanded. Returns the item immediately above the reference
	// item.
	//
    ...
    virtual HTREEITEM GetLastItem(
        // Node identifying the branch.  NULL will return the last item
		// in the outline.
        HTREEITEM hItem);
    ...
    virtual BOOL GetItemFont(
        // Handle of the reference item.
        HTREEITEM hItem,
        // Pointer to receive LOGFONT information.
        LOGFONT* plogfont);
    ...
    // This member function returns the RGB text value for the referenced
	// tree item, or (COLORREF)-1, if color was not set. Returns an RGB value
	// for the referenced tree item, or -1, if color was not set.
	//
    ...
    // This member function retrieves the first selected item in a multi-selection
	// tree control. Returns a handle to the first selected tree item.
	//
	HTREEITEM GetFirstSelectedItem() const;

    // This member function retrieves the number of tree items that are
	// selected. Returns a UINT representing the number of tree items selected.
	//
	...
	//
	void SelectAll(
		// TRUE to select all items, or FALSE to clear the selection.
		BOOL bSelect = TRUE);
    ...
    //
	void SelectItems(
        // The item to start selecting from.
		HTREEITEM hItemFrom,
        // The item to end selection at.
		HTREEITEM hItemTo,
	...
	//
	BOOL SelectChildren(
		// Handle of the tree item to begin selection from.
		HTREEITEM hParent,
	...
	// be made when the banding rect passes over it.  Returns the previous
	// banding state.
	//
	BOOL SetBandingHit(

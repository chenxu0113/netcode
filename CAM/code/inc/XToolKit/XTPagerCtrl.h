...
public:
    ...
    // Destroys a CXTPagerCtrl object, handles cleanup and de-allocation.
    //
...
public:
    ...
    //
    virtual BOOL Create(
	    // Specifies the window style attributes.  The following styles are
    ...
    //
    void ForwardMouse(
        // BOOL value that determines if mouse forwarding is enabled or
	...
	// control. Returns a COLORREF value that contains the previous background
	// color.     
    //
    COLORREF SetBkColor(
        // COLORREF value that contains the new background color of the
		// pager control.
        COLORREF clr);

    // This member function retrieves the current background color for
	// the pager control. Returns a COLORREF value that contains the current
	// background color.
    //
    COLORREF GetBkColor();

    // This member function sets the current border size for the pager
	// control. Returns an integer value that contains the previous border size,
	// in pixels.
    //
    int SetBorder(
        // New size of the border, in pixels.  This value should not be 
	...
	// control. Returns an integer value that contains the current border size,
	// in pixels.
    //
    int GetBorder();
    ...
    //
    void SetPos(
        // Integer value that contains the new scroll position, in pixels.        
        int iPos);

    // This member function retrieves the current scroll position of the
	// pager control. Returns an integer value that contains the current scroll
	// position, in pixels.
    //
    int GetPos();

    // This member function sets the current button size for the pager
	// control. Returns an integer value that contains the previous button size,
	// in pixels.
    //
    int SetButtonSize(
        // Integer value that contains the new button size, in pixels.
        int iSize);

    // This member function retrieves the current button size for the pager
	// control. Returns an integer value that contains the current button size,
	// in pixels.
    //
    int GetButtonSize();

    // This member function retrieves the state of the specified button
	// in a pager control. Returns the state of the button specified in 'iButton'.
	// See the return value description in PGM_GETBUTTONSTATE for a list of
	// possible values.
    //
    DWORD GetButtonState(
        // Indicates which button to retrieve the state for.  See the description
    ...
    //
    void GetDropTarget(
        // Address of an IDropTarget pointer that receives the interface

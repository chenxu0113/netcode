...
public:
    ...
    // Destroys a CXTFlatComboBox object, handles cleanup and de-allocation.
    //
    virtual ~CXTFlatComboBox();

protected:

    // This enum is used to determine the state the combo box should be.
	//
    enum STATE 
	{ 
		// Value to display the combo box as normal.
		NORMAL = 1,
		// Value to display the combo box as raised.
		RAISED = 2,
		// Value to display the combo box as pressed.
		PRESSED = 3
	};
	...
	BOOL m_bHasFocus;
	// Used by Autocompleting.
	BOOL m_bAutoComp;
	// TRUE if the control is flat.
	BOOL m_bFlatLook;
	// TRUE when autocomplete is disabled internally.
	BOOL m_bDisableAC;
...
public:
    ...
    void DisableFlatLook(
		// TRUE to disable flat appearance.
        BOOL bDisable);
	...
	// rect. Returns TRUE if the mouse is over the combo box, otherwise returns
	// FALSE.
    //
...
protected:
    ...
    virtual void DrawCombo(
        ...
        CDC* pDC,
		// Current state of the combo box, either NORMAL, RAISED, or PRESSED.
        STATE eState);
...
public:
	...
	// Destroys a CXTEdit object, handles clean up and de-allocation.
	//
	virtual ~CXTEdit();

protected:
	
	// Cool menu hook for this edit control.
	CXTCoolMenu m_coolMenu;
	// 'true' for initialization in OnCreate, set to 'true' for dynamically
	// created controls.
	bool m_bInitInCreate;

public:
	
	// Call this member function to determine if the Clipboard contains information
	// that can be pasted into this edit view. Returns TRUE if the Clipboard
	// contains data in a format which this edit view can accept, otherwise
	// it returns FALSE.
	//
	...
	// Call this member function to set the initialization for the edit control.
	...
	//
	void InitOnCreate(
		// 'true' to perform initialization in OnCreate.
		bool bInitInCreate);
...
public:
    ...
    // Destroys a CXTFlatEdit object, handles cleanup and de-allocation.
    //
	virtual ~CXTFlatEdit();

protected:

    // This enum is used to determine the state the edit control should be.
    enum STATE 
	{ 
		// Value to make the edit control normal.
		NORMAL = 1,
		// Value to make the edit control raised.
		RAISED = 2,
	};
...
public:
    ...
    //
    void DisableFlatLook(
		// TRUE to disable the flat appearance.
        BOOL bDisable);

    // This member function determines if the mouse is within the control's
	// rect. Returns TRUE if the mouse is over the edit box, otherwise returns
	// FALSE.
    //
    virtual BOOL PointInRect();
...
protected:
    ...
    virtual void DrawEdit(
		// Points to the current device context.
        CDC* pDC,
		// Current state of the edit control, either NORMAL or RAISED.
        STATE eState);
...
AFX_INLINE void CXTFlatEdit::DisableFlatLook(BOOL bDisable) {
    m_bFlatLook = !bDisable;
}
